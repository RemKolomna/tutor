...
this.validateForm = function(formObject, validationCallback, errorCallback, submitCallback) {
	...
	formObject.find("input,select,textarea").each(function() {
		...
		if(!skipValue) {
			// Check required
			if(inputObject.attr("data-required") && !fieldValue) {
				validationError(labelText + " обязательно");
				return false;
			}
			...
			if(dataType == "INTEGER") {
				var tmp = parseFloat(fieldValue);
				var res = Number.isInteger(tmp);
				if(!res) {
					validationError(labelText + ": число должно быть целым");
					return false;
				}

				var intValue = parseInt(fieldValue);
				if(isNaN(intValue)) {
					validationError(labelText + ": неверное значение");
					return false;
				}

				if(minValue && !isNaN(parseInt(minValue)) && intValue < parseInt(minValue)) {
					if(maxValue && !isNaN(parseInt(maxValue)))
						validationError(labelText + " должно быть между " + minValue + " и " + maxValue);
					else
						validationError(labelText + " должно быть равно или больше " + minValue);
					return false;
				}
				...
				if(maxValue && !isNaN(parseInt(maxValue)) && intValue > parseInt(maxValue)) {
					if(minValue && !isNaN(parseInt(minValue)))
						...
						validationError(labelText + " должно быть равно или меньше " + maxValue);
					return false;
				}
				fieldValue = intValue;
			}
			...
			if(dataType == "FLOAT")
			{
				...
				if(isNaN(floatValue)) {
					validationError(labelText + ": Введено неверное значение");
					return false;
				}
			...
			if(dataType == "EMAIL") {
				if(fieldValue && !isValidEmail(fieldValue)) {
					validationError(labelText + ": введите правильный адрес email");
					return false;
			    }
			}
			...
			if(dataType == "DATE") {
				if(fieldValue == "") {
					...
					if(!date.isValid()) {
						validationError(labelText + ": Введено неверное значение." + (dataFormat ? " Дата должна соответствовать формату \"" + dataFormat + "\"" : ""));
						return false;
					}
